...
#
#   python3 scripts/gen-social-preview.py --check      # diff only, for CI
#
# --check gates the SVG and DELIBERATELY NOT THE PNG, and needs no rsvg-convert.
# The SVG is deterministic text derived from modal-map.json, so a difference
# there is one this repository produced. The PNG is a rasterisation of that SVG:
# diffing its bytes would gate on the librsvg build the runner happens to carry
# rather than on anything galata computes, and would go red on an image that is
# correct. Not needing librsvg is also what lets the check run in CI at all:
# the dependency is why this artefact went ungated for so long, and why its
# caption sat a version behind from 1b403cf while every job stayed green.
...
if "--check" in sys.argv[1:]:
    if not svg.exists():
        sys.exit(f"{svg.relative_to(ROOT)} is missing; run this script without --check.")
    committed = svg.read_text()
    if committed != rendered:
        diff = difflib.unified_diff(
            committed.splitlines(), rendered.splitlines(),
            fromfile=f"{svg.relative_to(ROOT)} (committed)",
            tofile=f"{svg.relative_to(ROOT)} (regenerated)", lineterm="", n=1)
        sys.exit("\n".join([*diff, "",
                            "The committed card disagrees with what the chain produces. It is drawn",
                            f"from {DATA.relative_to(ROOT)}, so regenerate both together:",
                            "  scripts/gen-modal-map.sh build/dev/tools/social/galata-modal-map",
                            "  python3 scripts/gen-social-preview.py",
                            "The PNG is not compared here and must be regenerated by that command",
                            "rather than by hand -- it is a rasterisation, not a source."]))
    print(f"{svg.relative_to(ROOT)} matches {DATA.relative_to(ROOT)} ({len(MODES)} modes)")
    sys.exit(0)
