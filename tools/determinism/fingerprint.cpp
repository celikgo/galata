...
//
// The determinism fingerprint battery.
//
// Emits: a fixed battery of computations, each
...
void fingerprint_atmosphere(const Emit& emit) {
...
void fingerprint_rigid_body(const Emit& emit) {
...
void fingerprint_modes(const Emit& emit) {
...
void fingerprint_trim_and_linearisation(const std::string& model_path, const Emit& emit) {
  const model::Aircraft aircraft = model::load_aircraft(model_path);
...
Counts fingerprint_counts(const std::string& model_path) {
  ...
  fingerprint(model_path, [&counts](const std::string& key, double) {
    ++counts.total;
    // The tier-2 comparison skips exactly this prefix; see
    // scripts/compare-determinism.sh. Counting it the same way here means
    // the two cannot disagree about what is excluded.
    if (key.rfind("tier1.", 0) == 0) {
      ++counts.tier1_only;
    }
  });
  return counts;
