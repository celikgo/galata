...
### Discrete-time synthesis and sampled-loop margins: reconciled

These two are **outside the accepted bounded workflow**, and that is a conditional statement
rather than a free one. It holds only because of exactly what the delivered workflow claims,
so the claim is written out here and the condition attached to it.
...
discrete-time synthesis or a sampled margin, which is why their absence does not leave a
delivered result unsupported.

**What would move them back into scope.** Any of the following would make the claim exceed its
evidence, and each needs the missing machinery rather than more of the present machinery:

- Claiming the sampled loop is **stable**, rather than that this run converged.
- Claiming a **margin** for the sampled loop — gain, phase, delay or disk — from any
  continuous-domain figure. The delay comparison above is explicitly not this.
- Claiming stability or performance **across** initial conditions, weights, sample rates or
  delays, rather than at the one point the study declares.
- Designing the controller **in discrete time**, which is what `c2d` and a sampled DARE are
  for. `sim.sampled` executes a continuously-designed law at a discrete rate; that is the
  honest description and it is not the same operation.

**Neither is marked delivered anywhere.** They are absent from the capability registry, absent
from the README's generated table, and named as absent in RFC-0002's WP5 record, in
`docs/ROADMAP.md`, and in the list below. Per `docs/product/FEATURES.md`, this programme
implements the proposed work of F14 and part of F17 and delivers **neither row**; both stay
open, and an epic cannot pass acceptance while a required dependency is unresolved.
