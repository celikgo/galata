...
| Simulation and identification | Independently measured hardware responses, SITL/HIL interfaces, and the sampled loop's own robustness margins — a stabilising discrete design and a converging sampled run establish no margin, and no capability computes one. Measured-data import, grey-box identification, held-out validation, declared input histories, sampled control execution, discrete-time models and sampled LQR design (`model.discretize`, `synth.dare`, `synth.sampled_lqr`), reachability and observability analysis, and loop-at-a-time frequency-domain margins are implemented and UNVALIDATED — RFC-0002's WP3 to WP5 records say what each does not establish, and no capability in this row has been checked against a real aircraft |
