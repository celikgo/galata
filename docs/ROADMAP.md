...
| Simulation and identification | Independently measured hardware responses, SITL/HIL interfaces, discrete-time synthesis (`c2d`, sampled DARE) and the sampled loop's own robustness margins. Measured-data import, grey-box identification, held-out validation, declared input histories, sampled control execution, reachability and observability analysis, and loop-at-a-time frequency-domain margins are implemented and UNVALIDATED — RFC-0002's WP3 to WP5 records say what each does not establish, and no capability in this row has been checked against a real aircraft |
